...
from warm_start_paramenters_v2 import *
...
def main(): 
    ...
    parser.add_argument("--problem_type", help="problem type, QUBO or MIS", required=False, type=str, default='QUBO')
    ...
    with open(graph_file, 'rb') as f:
        G = pickle.load(f)
    #edge_list = list(G.edges) ## used in bond dimension 100
    ...
    if problem_type == 'QUBO':
        if if_analytic == 1:
            ...
            dir_0 = './data_iter_adap_sorting_measure/'
        ...
        if if_analytic == 1:
            dir_0 = './MISdata_iter_adap_sorting_analytic/'
        else:
            dir_0 = './MISdata_iter_adap_sorting_measure/'
        os.makedirs(dir_0, exist_ok=True)
        ...
        for i in range(n_qubits):
            d = G.degree[i]
            coeff_list.append(1/2 - lam/4 * d)
        for edge in G.edges:
            coeff_list.append(lam/4)

        const = -n_qubits/2 + lam/4 * len(G.edges)
        print('\nconst drop in MIS: ', const)
    ...
    n_qubits, tau, layer, expz_array, edge_coeff_dict, edge_list, eigen_idvalue_dict, shots, backendoptions, if_analytic)     #HO SPOSTATO IL SALVADATI A DOPO
    ...
    if if_adsorting:
        for Abs in [True, False]:
            for invert in [True, False]:
                ...
                edge_params_dict, params_init, exp_poss_dict = MimicITE_iterate(\
                n_qubits, tau, layer, expz_array, edge_coeff_dict, sorted_edge_list, eigen_idvalue_dict, shots, backendoptions, if_analytic)     #HO SPOSTATO IL SALVADATI A DOPO
                cvar, expz_array = get_expz(n_qubits, exp_poss_dict['l_1'] , alpha)
