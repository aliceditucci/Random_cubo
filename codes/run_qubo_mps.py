...
# from codes.mps_functions import * 
...
def main(): 
    ...
    parser.add_argument("--graph_type", help="type of graph, complete or 3regular", required=False, type=str, default='3regular')
    ...
    #make data dir
    if if_analytic == 1:
        dir_0 = './data_iter_adap_sorting_analytic_mps/'
        
    else:
        dir_0 = './data_iter_adap_sorting_nomeasure_mps/'
    ...
    if if_adsorting:
        for Abs in [True, False]:
            for invert in [True, False]:
                ...
                print('(exp, poss):', [(float(tp[0][0]), tp[1], float(tp[0][1])) for tp in list(exp_poss_dict['l_1'].items())[:5]])
                # print('expz_array: ', expz_array)
